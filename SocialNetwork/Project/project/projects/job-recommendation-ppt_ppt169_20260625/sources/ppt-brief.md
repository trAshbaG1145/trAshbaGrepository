...
| 模块 | 端口 | 作用 |
| --- | --- | --- |
| MySQL (Docker) | 3413 | 保存用户、角色、岗位卡片等业务数据 |
| Redis (Docker) | 6380 | 后端缓存/会话相关依赖 |
| Neo4j (Docker) | 7475 HTTP / 7688 Bolt | 保存职业知识图谱 |
| Flask | 8081 | 提供图谱检索、推荐、AI 问答、时间线和简历分析接口 |
| Spring Boot | 8090 | 提供注册登录、用户信息、上传、岗位卡片等业务接口 |
| Vite 前端 | 8089 | 浏览器演示入口 |

> **一键启动**：项目根目录提供 `start-project.bat`，自动启动 Docker Compose 基础设施和所有服务进程，无需手动逐个启动。
